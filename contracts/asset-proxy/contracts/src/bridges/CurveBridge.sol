    ...
    DeploymentConstants
{
        ...
        returns (bytes4 success)
    {
        ...
        require(fromTokenAddress == _getDaiAddress() || toTokenAddress == _getDaiAddress(), "CurveBridge/INVALID_ADDRESS");

        ICurve exchange = ICurve(_getCurveAddress());
        // Grant an allowance to the exchange to spend `fromTokenAddress` token.
        LibERC20Token.approve(fromTokenAddress, address(exchange), uint256(-1));

        // Try to sell all of this contract's `fromTokenAddress` token balance.
        exchange.exchange_underlying(
            fromTokenAddress == _getDaiAddress() ? 0 : 1,
            fromTokenAddress == _getDaiAddress() ? 1 : 0,
            // dx
